...
function HolderQuickStats({ tokens }: { tokens: Token[] }) {
  const holderCounts = tokens.map(t => Number(t.holderCount || 0));
  const totalHolders = holderCounts.reduce((a, b) => a + b, 0);
  const highestToken = tokens.reduce((max, t) => Number(t.holderCount || 0) > Number(max.holderCount || 0) ? t : max, tokens[0]);
  const lowestToken = tokens.reduce((min, t) => (Number(t.holderCount || Infinity) < Number(min.holderCount || Infinity) && Number(t.holderCount || 0) > 0) ? t : min, tokens[0]);
  const avgHolders = tokens.length ? totalHolders / tokens.length : 0;
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Highest Holders</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{highestToken?.symbol || "N/A"}</div>
          <p className="text-xs text-muted-foreground">{formatNumber(Number(highestToken?.holderCount || 0))} holders</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Lowest Holders</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{lowestToken?.symbol || "N/A"}</div>
          <p className="text-xs text-muted-foreground">{formatNumber(Number(lowestToken?.holderCount || 0))} holders</p>
        </CardContent>
      </Card>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Average Holders</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{formatNumber(avgHolders)}</div>
...
export default function HoldersPage() {
    ...
    const fetchTokens = async () => {
      ...
      try {
        const response = await fetch(
          `https://liquidlaunch.app/api/tokens?page=${page}&limit=15&search=&sortKey=holders&sortOrder=desc&timeframe=24h&view=in_progress&marketCapMin=0&marketCapMax=1000000&progressMin=0&progressMax=100&filterByHolderCount=false`
        )
        ...
        const processedData = data.tokens
          ...
          .sort((a: ChartData, b: ChartData) => b.holders - a.holders)
          .slice(0, 10)
  ...
  // Custom Bar label to show logo above the bar, round image
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <Card>
          <CardHeader>
            <div className="space-y-1">
              <h2 className="text-3xl font-bold tracking-tight">Holder Distribution</h2>
              <p className="text-muted-foreground">Top 10 tokens by number of holders</p>
            </div>
          </CardHeader>
          <CardContent>
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 44, right: 30, left: 20, bottom: 20 }}>
                      ...
                      return (
                        <div className="bg-background border rounded-lg p-3 shadow-lg">
                          ...
                          <p className="text-primary font-semibold mt-1">
                            Holders: {formatNumber(data.holders)}
                          </p>
                  ...
                  <Bar dataKey="holders" fill="hsl(var(--primary))" label={CustomBarWithLogo} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        {/* Quick Stats Section */}
        {!loading && <HolderQuickStats tokens={tokens} />}

        {/* Token List Section */}
        <Card>
          <CardHeader>
            <CardTitle>Top Holder Tokens</CardTitle>
            <p className="text-muted-foreground text-sm mt-1">
              This page shows tokens with the highest number of holders.
            </p>
          </CardHeader>
          <CardContent>
            {loading ? (
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center bg-primary/5 rounded-lg px-2 py-1 mt-2 mb-1">
                          ...
                          <div className="flex flex-col items-center">
                            ...
                            <span className="text-lg font-bold text-primary">{formatNumber(holderCount)}</span>
                          </div>
                          <div className="flex flex-col items-center">
