...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    ...
    // Fetch token data for each address in watchlist
    const fetchTokens = async () => {
      ...
      try {
        ...
        // Filter tokens to only include those in watchlist
        const watchlistTokens = data.tokens.filter((token: any) => 
          savedWatchlist.includes(token.address)
        )
  ...
  const removeFromWatchlist = (address: string, e: React.MouseEvent) => {
    e.stopPropagation()
    const newWatchlist = watchlist.filter(addr => addr !== address)
          ...
          <CardContent className="p-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                No tokens in your watchlist. Add tokens by clicking the star icon on any token card.
              </div>
            ) : (
                      ...
                      <div className="flex justify-between items-center mt-2 text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            ...
                            onClick={(e) => removeFromWatchlist(token.address, e)}
